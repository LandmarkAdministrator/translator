...
- **GPU accelerated** — AMD ROCm or NVIDIA CUDA **(required — CPU-only operation is not supported)**
...
Audio devices, enabled languages, and the ASR model are configured interactively with `python run.py --setup` and saved to `config/settings.yaml`. Edit that file directly (or re-run `--setup`) to change settings — there is no separate per-language YAML to edit.
...
├── config/
│   └── settings.yaml           # Generated by --setup (devices, languages, ASR model)
├── src/
